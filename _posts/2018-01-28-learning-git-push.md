...
```bash
[user@localhost]$ git push <远程主机名> <本地分支名>:<远程分支名>
```
...
```bash
[user@localhost]$ git push origin :master
[user@localhost]$ git push origin --delete master
```
...
```bash
[user@localhost]$ git config --global push.default matching
[user@localhost]$ git config --global push.default simple
```

还有一种情况，就是不管是否存在对应的远程分支，将本地的所有分支都推送到远程主机，这时需要使用–all选项。

```bash
[user@localhost]$ git push --all origin
```
...
```bash
[user@localhost]$ git push --force origin
```
...
```bash
[user@localhost]$ git push origin --tags
```